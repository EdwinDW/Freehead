<?php
  ...
  $gatherer_objects = array(
    ...
    73941, 123309, 123310, 123848, 165658, 175404, 177388, 180215, 181555,
    181556, 181557, 181569, 185877, 189978, 189979, 189980, 189981, 191133,
    // HERB
    1617, 1618, 1619, 1620, 1621, 1622, 1623, 1624, 1628, 2041, 2042, 2043,
    2044, 2045, 2046, 2866, 142140, 142141, 142142, 142143, 142144, 142145,
    176583, 176584, 176586, 176587, 176588, 176589, 181166, 181270, 181271,
    181275, 181276, 181277, 181278, 181279, 181280, 181281, 185881, 189973,
    190169, 190170, 190171, 190172, 190175, 190176, 191019, 191303,
    // OPEN
    2039, 2744, 2843, 2844, 2850, 3658, 3659, 3660, 3661, 3662, 3705, 3706,
    3714, 19019, 28604, 74447, 74448, 75293, 123330, 131978, 131979, 142191,
    157936, 164958, 176213, 176582, 178244, 179486, 179487, 179492, 179493,
    179498, 181628, 182053, 164881, 164882, 174662, 164884, 185881, 185877,
    179697, 182069, 142184, 177784, 2560, 141931, 2883, 152608, 105176,
    123330, 181665, 184793, 105570, 179486, 179487, 179492, 179493, 184740,
    184742, 181798, 181800, 181802, 181804, 185915, 182258, 181598, 193997,
  );

  $gatherer_areas = array(
  ...
  $version = @$DB->selectCell('SELECT db_version FROM version');
  if (!$version)
    $version = @$DB->selectCell('SELECT version FROM db_version');
  if (!$version)
    $version = "FreedomHead database";
  ...
  foreach($gatherer_objects as $object)
  {
    ...
    foreach($positions as $area)
      ...
      {
        $areaname = $gatherer_areas[$area['atid']];
        if (!isset($gathererdb[$areaname]))
          $gathererdb[$areaname] = array();
        if (!isset($gathererdb[$areaname][$object]))
          $gathererdb[$areaname][$object] = array();
        foreach($area['points'] as $point)
        {
          $x = $point['x'];
          $y = $point['y'];
          if ($x >= 0 && $x <= 99.9 && $y >= 0 && $y <= 99.9)
            $gathererdb[$areaname][$object][] = sprintf("%03d%03d", $x*10, $y*10);
        }
      }

    // Workaround for buggy position() function, should be fixed in game.php
...
";
  ...
  {
    $gathererdata .= "\t$areaname = {\n";
    foreach($areainfo as $object => $list)
    {
      $gathererdata .= "\t\t[$object] = {";
...
//  print($gathererdata);
  ...
  {
    $zip = new ZipArchive();
    if ($zip->open('GathererDB_FreedomHead.zip', ZIPARCHIVE::OVERWRITE) !== TRUE)
  ...
  else
  {
    class ZipFallback
    {
      function addFromString($filename, $data)
      {
        @mkdir(dirname($filename), 0777, true); // suppress "Directory exists" warning
